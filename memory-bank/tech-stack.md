...
### 4.3 棋盘绘制策略（正式版建议）
- 棋子应绘制在**交叉点**而非格子内部（符合中国象棋标准视图）
- 建议使用 `iced` 的 `canvas` 来绘制：
  - 棋盘线（含河界断线）
  - 九宫斜线
  - 选中/推荐走法高亮
  - 棋子圆盘与中文字符
...
- 可在不新增依赖的前提下实现最小 HTTP 查询（例如使用 `TcpStream`）
...
- 已完成实施计划 Step 6（`xq-app` iced UI + 最小可用串联）：
  - iced 主布局落地（工具栏 + 棋盘 + 侧栏），侧栏支持“分析/棋谱/提示”；
  - 棋盘使用 `canvas` 绘制，棋子落在交叉点，并绘制九宫斜线与河界断线；
  - 连线模式 UI 可选择外部窗口、刷新列表、开始/停止连线；
  - 连线循环接入 `xq-link + xq-vision`，识别结果驱动棋盘更新；
  - 引擎与云库（chessdb）已接入：云库命中优先，失败回退本地引擎；
  - 人机对弈/AI vs AI 使用引擎输出的 bestmove 进行最小可用落子。
